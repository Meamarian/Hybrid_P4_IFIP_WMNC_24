...
#define MAX_CLIENTS 8  // as far as ports
...
struct client {
    struct rte_ring *ring; // Pointer to the client's ring
    struct client_rx_buf rx_buf; // Client's receive buffer
};
...
//void handle_packet(struct rte_mbuf *m,struct rte_ring *send_ringx,unsigned portid,int clone_idx) {
// void handle_packet(struct rte_mbuf *m,unsigned portid) {
struct rte_mbuf* handle_packet(struct rte_mbuf *m,unsigned portid) {
...
void flush_rx_queue(uint8_t client_id)
{
    struct client *cl = &clients[client_id];

    if (cl->rx_buf.count == 0)
        return;

    if (rte_ring_enqueue_bulk(cl->ring, (void **)cl->rx_buf.buffer,
                              cl->rx_buf.count, NULL) == 0) {
        for (uint16_t j = 0; j < cl->rx_buf.count; j++) {
            rte_pktmbuf_free(cl->rx_buf.buffer[j]);
        }
    }
...
void enqueue_rx_packet(uint8_t client_id, struct rte_mbuf *buf)
{
    struct client *cl = &clients[client_id];
    cl->rx_buf.buffer[cl->rx_buf.count++] = buf;
}
...
l2fwd_main_loop(void)
{
	...
	// // Initialize cl_rx_buf
    // cl_rx_buf = malloc(sizeof(struct client_rx_buf));
    // if (!cl_rx_buf) {
    //     fprintf(stderr, "Failed to allocate client_rx_buf\n");
    //     return -1;
    // }
    // cl_rx_buf->count = 0;
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				...
				enqueue_rx_packet(portid,m);
				// if (++client == MAX_CLIENTS)
				// 	client = 0;
			
				flush_rx_queue(portid);
				ring_statistics[portid].tx += 1;

				// for (t = 0; t < MAX_CLIENTS; i++)
				// 	flush_rx_queue(t);
...
// 	unsigned socket_id;
// 	const char * q_name;
// 	const unsigned ringsize = 2097152*8*16;;
// 	int num_clients=1 ;
...
// 		q_name = 'ring0';
// 		clients[i].rx_q = rte_ring_create(q_name,ringsize, socket_id, flags ); /* single prod, single cons */
...
// }
	...
	const unsigned ring_size = 2097152*1*16;
    //send_ring0 = rte_ring_create("ring0", ring_size, rte_socket_id(), flags);

    for (int i = 0; i < MAX_CLIENTS; i++) {
        char ring_name[32];
        snprintf(ring_name, sizeof(ring_name), "ring%d", i);
        clients[i].ring = rte_ring_create(ring_name, ring_size, rte_socket_id(), flags);
        if (!clients[i].ring) {
            rte_exit(EXIT_FAILURE, "Failed to create ring for client %d\n", i);
        }
        clients[i].rx_buf.count = 0;
    }
	...
	// send_ring6 = rte_ring_create("ring6"å, ring_size, rte_socket_id(), flags);
	...
	populate_teid_qfi_table("/home/admin/mohsen/dpdk/examples/gnb/config_table1_64000_0_table2_10.0.0.0_10.0.250.0_0.txt");
    ...
    // Call init_shm_rings and handle any errors
    // ret = init_shm_rings();
    // if (ret < 0) {
    //     fprintf(stderr, "Error: Failed to initialize shared memory rings\n");
	...
	//ßrte_eal_mp_remote_launch(l2fwd_launch_one_lcore, NULL, SKIP_MASTER);
