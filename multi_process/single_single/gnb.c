...
flush_rx_queue(uint16_t client)
{
	...
	if (cl_rx_buf->count == 0)
		return;

	//cl = &clients;
	if (rte_ring_enqueue_bulk(send_ring0, (void **)cl_rx_buf->buffer,
			cl_rx_buf->count, NULL) == 0){
		for (j = 0; j < cl_rx_buf->count; j++)
			rte_pktmbuf_free(cl_rx_buf->buffer[j]);
		//cl->stats.rx_drop += cl_rx_buf->count;
	}
	else
		//cl->stats.rx += cl_rx_buf->count;
...
{
	cl_rx_buf->buffer[cl_rx_buf->count++] = buf;
}
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				...
				//enqueue_rx_packet(client,m);
				//flush_rx_queue(client);
				l2fwd_simple_forward(m, portid);
				// if (rte_ring_enqueue(send_ring0, (void *)m) < 0) {
				// 	// Handle enqueue failure
				// 	printf("Failed to enqueue the packet\n");
				// 	rte_pktmbuf_free(m);
				// } else {
