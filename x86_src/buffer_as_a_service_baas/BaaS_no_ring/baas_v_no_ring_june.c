...
static uint8_t resubmission_time = 1; //MS
...
handle_incoming_packet(struct rte_mbuf * m, struct bucket_t * current_bucket, struct rte_hash* buffer_table, struct rte_hash* buffer_table_teid, uint8_t portid) {
   //RTE_LOG(INFO, L2FWD, "NOTE2, innner handle pkt\n");
    ...
    if (src_udp_port == 8040) {
    // RTE_LOG(INFO, L2FWD, "NOTE, RLC status arrived\n");
    uint32_t teid1 = rte_be_to_cpu_32(*(uint32_t *)(pkt_data + 14 + 20 + 8 + 4)); 
    uint32_t seq_num = rte_be_to_cpu_16(*(uint16_t *)(pkt_data + 14 + 20 + 8 + 1)); 
    //uint32_t packet_id1 = teid;
    //RTE_LOG(INFO, L2FWD, "N, rlc teid 0x%.16" PRIX64" \n", teid1);

        struct packet_in_buffer_t* pointer_to_packet_in_bucket1;
        int ret1 = rte_hash_lookup_data(buffer_table, &teid1, (void**)&pointer_to_packet_in_bucket1);

        if(ret1 >= 0) {
            // remove_packet_from_bucket(current_bucket, pointer_to_packet_in_bucket1);
            //RTE_LOG(INFO, L2FWD, "N, ACK arrived teid  0x%.16" PRIX64 "\n", teid1);
            rte_pktmbuf_free(pointer_to_packet_in_bucket1);
 
            port_statistics[portid].deleted += 1;
            rte_hash_del_key(buffer_table, &teid1);
        }else{
    ...
    rte_pktmbuf_free(m);
    ...
    } if(src_udp_port == 2152) {
        //RTE_LOG(INFO, L2FWD, "NOTE, GTP arrived\n");
        uint32_t teid = rte_be_to_cpu_32(*(uint32_t *)(pkt_data + 14 + 20 + 8 + 4)); 
        uint32_t seq_num = rte_be_to_cpu_16(*(uint16_t *)(pkt_data + 14 + 20 + 8 + 1)); 
        //uint32_t packet_id = teid;
        //RTE_LOG(INFO, L2FWD, "N, gtp teid 0x%.16" PRIX64" \n", teid);
        // RTE_LOG(INFO, L2FWD, "N, rlc teid 0x%.16" PRIX64" \n", seq_num);
       ...
       //pointer_to_packet_in_bucket = insert_packet_to_bucket(current_bucket, m, portid, packet_id);
        ...
        // }else{
            ...
            */
            int ret1 = rte_hash_add_key_data(buffer_table, &teid, (void * )m);
            ...
            if (ret1 == 22){
                ...
                //RTE_LOG(INFO, L2FWD, "NOTE, insertion ok\n");
            }
